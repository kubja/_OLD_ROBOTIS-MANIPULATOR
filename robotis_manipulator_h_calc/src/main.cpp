...
void* thread_20ms_proc( void* arg )
{
	...
	while( ros::ok() )
	{
        ...
        {
            _joint_values[ id ]	=	JointState::m_goal_joint_state[ id ].m_position;
            joint_values[ id ]	=	JointState::m_goal_joint_state[ id ].m_position;
        }
        ...
        if ( collision_result.collision == true )
        {
            ...
            for ( int id = 1; id <= MAX_JOINT_ID; id++ )
                temp_position[ id ] = _joint_values[ id ];
        ...
        {
            for ( int id = 1; id <= MAX_JOINT_ID; id++ )
                temp_position[ id ] = joint_values[ id ];
        ...
        if ( ManipulatorH.cnt >= ManipulatorH.all_time_steps && task_20m_running == true )
		{
            ...
            for( std::size_t id = 0; id < joint_names.size(); id++ )
                ROS_INFO("%s: %f", joint_names[ id ].c_str(), joint_values[ id ]);
